...
public class PageServiceImpl extends TransactionalService implements PageService, ApplicationContextAware {
	...
	public List<String> validateSafeContent(String content, io.gravitee.repository.management.model.PageType type) {
        ...
        if (markdownSanitize && io.gravitee.repository.management.model.PageType.MARKDOWN.toString().equals(type.toString())) {
        ...
        } else if (type != null && io.gravitee.repository.management.model.PageType.SWAGGER.toString().equals(type.toString())) {
			SwaggerParseResult swaggerParseResult = new OAIParser().parse(content);
			if (swaggerParseResult != null && swaggerParseResult.getMessages() != null) {
