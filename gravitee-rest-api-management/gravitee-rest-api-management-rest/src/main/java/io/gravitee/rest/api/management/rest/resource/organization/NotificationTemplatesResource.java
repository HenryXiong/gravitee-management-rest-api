...
public class NotificationTemplatesResource extends AbstractResource {
    ...
    @ApiResponses({
            @ApiResponse(code = 200, message = "Notifications templates", responseContainer = "List" ,response = NotificationTemplateEntity.class),
            @ApiResponse(code = 500, message = "Internal server error")})
    @Permissions({
            @Permission(value = RolePermission.ORGANIZATION_NOTIFICATION_TEMPLATES, acls = RolePermissionAction.READ)
    })
    public Response getNotificationTemplates(
            @ApiParam(value = "filter by notification type")
            @QueryParam("type")
        ...
        if (type == null) {
            ...
            final Set<NotificationTemplateEntity> allByType = notificationTemplateService.findByType(type);
            return Response.ok(allByType).build();
        }
    }
    ...
    public Response getNotificationTemplate(
            @ApiParam(value = "ID of the notification template")
            @PathParam("notificationTemplateId")
    ...
    @ApiResponses({
            ...
            @ApiResponse(code = 500, message = "Internal server error")})
    @Permissions({
            @Permission(value = RolePermission.ORGANIZATION_NOTIFICATION_TEMPLATES, acls = RolePermissionAction.UPDATE)
    })
    public Response updateNotificationTemplate(
        ...
        final NotificationTemplateEntity updatedNotificationTemplate = notificationTemplateService.createOrUpdate(notificationTemplateEntityUpdate);
