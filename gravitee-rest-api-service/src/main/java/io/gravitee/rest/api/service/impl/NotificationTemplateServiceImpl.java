...
import static io.gravitee.repository.management.model.Audit.AuditProperties.NOTIF_TEMPLATE;
...
public class NotificationTemplateServiceImpl extends AbstractService implements NotificationTemplateService, InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        this.fromFilesNotificationTemplateEntities = allHooks.stream()
    ...
    private Map<String, NotificationTemplateEntity> addExtraHtmlTemplate() {
        try {
            return Files.list(new File(templatesPath).toPath())
...
//        configuration.setTemplateConfigurations(new ConditionalTemplateConfigurationFactory(new FileExtensionMatcher(HTML_TEMPLATE_EXTENSION), tcHTML));
    ...
    public Set<NotificationTemplateEntity> findAll() {
        ...
        final Set<NotificationTemplateEntity> allFromDatabase = this.findAllInDatabase(GraviteeContext.getCurrentOrganization(), NotificationTemplateReferenceType.ORGANIZATION);
    ...
    private Set<NotificationTemplateEntity> findAllInDatabase(String referenceId, NotificationTemplateReferenceType referenceType) {
        try {
            ...
            throw new TechnicalManagementException(
                    "An error occurs while trying to retrieve notificationTemplates", ex);
        }
    }
    ...
    private NotificationTemplateEntity loadPortalNotificationTemplateFromHook(Hook hook) {
        ...
        try {
            ...
            return createNotificationTemplateEntity(templateName,
                    hook.getLabel(),
                    load.get("title"),
                    load.get("message"),
                    hook.getDescription(),
    ...
    @NotNull
    private NotificationTemplateEntity createNotificationTemplateEntity(String templateId, String templateName, String title, String content, String description, io.gravitee.rest.api.model.notification.NotificationTemplateType type) {
        NotificationTemplateEntity newNotificationTemplateEntity = new NotificationTemplateEntity();
        newNotificationTemplateEntity.setId(templateId);
        newNotificationTemplateEntity.setType(type);
        newNotificationTemplateEntity.setTitle(title);
        newNotificationTemplateEntity.setContent(content);
        newNotificationTemplateEntity.setCreatedAt(new Date());
        newNotificationTemplateEntity.setName(templateName);
        newNotificationTemplateEntity.setDescription(description);
        newNotificationTemplateEntity.setScope(computeScopeFromId(templateId));

        return newNotificationTemplateEntity;
    }
    ...
    public Set<NotificationTemplateEntity> findByType(io.gravitee.rest.api.model.notification.NotificationTemplateType type) {
        try {
            return notificationTemplateRepository
                    .findAllByType(NotificationTemplateType.valueOf(type.name()), GraviteeContext.getCurrentOrganization(), NotificationTemplateReferenceType.ORGANIZATION)
                    .stream()
                    .map(this::convert)
                    .collect(Collectors.toSet());
        } catch (TechnicalException ex) {
            ...
            throw new TechnicalManagementException(
                    "An error occurs while trying to retrieve notificationTemplates by type", ex);
        }
    }

    @Override
    public NotificationTemplateEntity createOrUpdate(NotificationTemplateEntity newNotificationTemplate) {
        try {
            LOGGER.debug("Create notificationTemplate {}", newNotificationTemplate);

            Optional<NotificationTemplate> optNotificationTemplate = notificationTemplateRepository.findById(newNotificationTemplate.getId(), GraviteeContext.getCurrentOrganization(), NotificationTemplateReferenceType.ORGANIZATION);
            NotificationTemplate notificationTemplate = convert(newNotificationTemplate);

            NotificationTemplate createdOrUpdatedNotificationTemplate = null;
            if (optNotificationTemplate.isPresent()) {
                createdOrUpdatedNotificationTemplate = notificationTemplateRepository.update(notificationTemplate);
                createAuditLog(NotificationTemplate.AuditEvent.NOTIFICATION_TEMPLATE_UPDATED, notificationTemplate.getUpdatedAt(), optNotificationTemplate.get(), notificationTemplate);
            } else {
                createdOrUpdatedNotificationTemplate = notificationTemplateRepository.create(notificationTemplate);
                createAuditLog(NotificationTemplate.AuditEvent.NOTIFICATION_TEMPLATE_CREATED, notificationTemplate.getCreatedAt(), null, notificationTemplate);
            }

            final NotificationTemplateEntity createdOrUpdatedNotificationTemplateEntity = convert(createdOrUpdatedNotificationTemplate);

            // Update template in loader cache
            updateFreemarkerCache(createdOrUpdatedNotificationTemplateEntity);

            return createdOrUpdatedNotificationTemplateEntity;
        } catch (TechnicalException ex) {
    ...
    public NotificationTemplateEntity findById(String id) {
        try {
            ...
            Optional<NotificationTemplate> notificationTemplate = notificationTemplateRepository.findById(id, GraviteeContext.getCurrentOrganization(), NotificationTemplateReferenceType.ORGANIZATION);

            if (notificationTemplate.isPresent()) {
                return convert(notificationTemplate.get());
            } else {
                NotificationTemplateEntity notificationTemplateEntity = fromFilesNotificationTemplateEntities.get(id);
            ...
            throw new TechnicalManagementException(
                    "An error occurs while trying to delete a notificationTemplate using its ID " + id, ex);
        }
    }
    ...
    private void createAuditLog(Audit.AuditEvent event, Date createdAt, NotificationTemplate oldValue, NotificationTemplate newValue) {
        ...
        auditService.createOrganizationAuditLog(
                Collections.singletonMap(NOTIF_TEMPLATE, notificationTemplateName),
                event,
                createdAt,
                oldValue,
                newValue
    ...
    private NotificationTemplateEntity convert(NotificationTemplate notificationTemplate) {
        ...
        notificationTemplateEntity.setType(io.gravitee.rest.api.model.notification.NotificationTemplateType.valueOf(notificationTemplate.getType().name()));
        ...
        notificationTemplateEntity.setScope(computeScopeFromId(notificationTemplate.getId()));
        return notificationTemplateEntity;
    }

    private String computeScopeFromId(String id) {
        final String[] idItems = id.split("\\.");
        if (idItems.length == 2) {
            return "TEMPLATES TO INCLUDE";
        } else {
            return idItems[0].replaceAll("_", " ");
        }
    }
}
